...
function NewProduct() {
  ...
  const [differentSizes, setDifferentSizes] = useState("No")
  const [differentColors, setDifferentColors] = useState("No")
  ...
  const addProduct = () => {
    if (name === undefined | name.length == 0) {
      ...
      axios.post(`${API}/admin/add-product`, form)
        .then((res) => {
          alert(res.data)
        })
    }
